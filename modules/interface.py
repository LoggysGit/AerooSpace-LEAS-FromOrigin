...
class SettingsWindow(QWidget):
    ...
    def update_json_status(self):
        ...
        status = all(required)
        settings_path = lib.get_path(os.path.join("resources", "settings.json"))
        try:
            data = {}
    ...
    def save_to_env(self, key, value):
        try:
            set_key(lib.get_path(".env"), key, value)
...
class AnalyticsWindow(QWidget):
    def __init__(self, file_path):
        ...
        self.setStyleSheet(lib.STYLESHEET)
...
class AerooSpaceApp(QWidget):
    ...
    def load_fonts(self):
        font_path_krona = lib.get_path(os.path.join("assets", "fonts", "KronaOne.ttf"))
        font_path_jb = lib.get_path(os.path.join("assets", "fonts", "JetBrainsMono", "JetBrainsMono-Regular.ttf"))
    ...
    def setup_spaceports(self, combo):
        SPACEPORT_PATH = lib.get_path(os.path.join("resources", "spaceports.json"))
    ...
    def refresh_history(self):
        ...
        if not os.path.exists(lib.REPORTS_PATH):
            os.makedirs(lib.REPORTS_PATH)
            return

        # Filter and sort files by timestamp (Newest first)
        files = [os.path.join(lib.REPORTS_PATH, f) for f in os.listdir(lib.REPORTS_PATH) if f.endswith('.json')]
    ...
    def start_analyzing(self, btn):
        try:
            ...
            model_check_path = lib.get_path(os.path.join("assets", "model", "Qwen2.5-7B-Instruct-Q4_K_M.gguf"))
    ...
    def on_analysis_finished(self, file_name, btn):
        try:
            current_analytics_path = os.path.join(lib.REPORTS_PATH, file_name)
    ...
    def init_ui(self):
        ...
        sett_icon_path = lib.get_path(os.path.join("assets", "ui", "settings.svg"))
